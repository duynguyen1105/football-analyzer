...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `Vua phá lưới ${league.name} — Top ghi bàn mùa giải 2025/26`,
    description: `Bảng xếp hạng vua phá lưới ${league.name} mùa giải 2025/26. Thống kê bàn thắng, kiến tạo chi tiết.`,
  };
}
...
export default async function TopScorersPage({ params }: Props) {
  ...
  const players = await getTopScorers(league.code);

  return (
    <section>
      <h2 className="text-base font-bold mb-2">Vua phá lưới {league.name}</h2>
      <p className="text-sm text-text-secondary mb-6">
        Bảng xếp hạng ghi bàn mùa giải 2025/26
      </p>
          ...
          {players.map((player, i) => (
            ...
            >
              ...
              <div className="flex gap-4 text-center">
                ...
                <div>
                  <p className="text-lg font-bold text-text-secondary">{player.assists ?? 0}</p>
